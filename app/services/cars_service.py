...
class CarSegmentationService:
    """
    Adapter for a trained YOLO segmentation model that predicts the car mask.
    ...
    @staticmethod
    def _fallback_car_polygon() -> np.ndarray:
        return np.array(
            [
                [0.04, 0.16],
                [0.94, 0.12],
    ...
    def _fallback_payload(self, width: int, height: int) -> list[dict[str, object]]:
        poly_norm = self._fallback_car_polygon()
        poly_px = np.stack(
        ...
        return [
            {
                "model_name": "car_segmentation_stub",
                "class_id": 0,
